...
class TuVan():
    ...
    def process(self, input_user):
        if input_user=='Tư vấn bệnh':
        ...
        elif(self.correct_input(input_user)=='Error'):
            ...
            if input_user!='Tiếp tục' and input_user !='Dừng':
                ...
                if self.new_turn== False:
                    ...
                    id_trieuchung_user = self.correct_input(input_user)
    ...
    def correct_input(self,input_user):
        ...
        if self.id_question =='Q_error':
            if input_user=='Tiếp tục':
                ...
                return 'Error'
        if input.predict(self.id_question,input_user)== None:
            return 'Error' 
        else:
            # return  list(self.df_trieuChung[self.df_trieuChung['id']==input.predict(self.id_question,input_user)]['ten'].values)[0]
            return input.predict(self.id_question,input_user)
